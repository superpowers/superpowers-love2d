...
export default class Loved2BuildSettingsEditor implements SupClient.BuildSettingsEditor {
  ...
  constructor(container: HTMLDivElement, private entries: SupCore.Data.Entries, private entriesTreeView: TreeView) {
    ...
    const outputFolderRow = SupClient.table.appendRow(tbody, SupClient.i18n.t("buildSettingsEditors:love2d.outputFolder"));
  ...
  getSettings(callback: (settings: Love2dBuildSettings) => void) {
  ...
  private ensureOutputFolderValid(callback?: (outputFolderValid: boolean) => void) {
    if (outputFolder == null) {
      this.displayError(SupClient.i18n.t("buildSettingsEditors:love2d.errors.selectDestionationFolder"));
    ...
    SupApp.readDir(outputFolder, (err, files) => {
      if (err != null) {
        this.displayError(SupClient.i18n.t("buildSettingsEditors:love2d.errors.emptyDirectoryCheckFail"));
        console.log(err);
        if (callback != null) callback(false);
        return;
      }
      ...
      if (files.length > 0) {
        this.displayError(SupClient.i18n.t("buildSettingsEditors:love2d.errors.destinationFolderEmpty"));
        if (callback != null) callback(false);
      } else {
        this.errorRowElt.hidden = true;
